...
const M_COLORS = { Weaning:"bg-gray-400", Growing:"bg-amber-500", "Near Maturity":"bg-orange-500", Ready:"bg-green-600" };

function MaturityBar({current,target}) {
  if(!current||!target) return null;
  const pct=Math.min(100,Math.round((parseFloat(current)/parseFloat(target))*100));
  const col=pct<50?"bg-amber-400":pct<80?"bg-orange-500":"bg-green-500";
...
export default function BucklingProfile() {
  const {id}=useParams(); const navigate=useNavigate();
  const {animals=[],setAnimals}=useContext(FarmContext);
  const bk=animals.find((a)=>a.id?.toString()===id&&a.category==="goats"&&a.type?.toLowerCase()==="buckling");
  ...
  if(!bk) return <p className="p-6 text-red-600">Buckling not found.</p>;
  const gr=bk.growthRecords||[]; const fr=bk.feedRecords||[]; const ml=bk.medicalLog||[]; const dr=bk.drenchingRecords||[];
  ...
  const startEdit=()=>{setIsEditing(true);setEdited({...bk});}; const cancelEdit=()=>{setIsEditing(false);setEdited(null);};
  const saveEdit=()=>{upd(edited);setIsEditing(false);setEdited(null);}; const uf=(f,v)=>setEdited((p)=>({...p,[f]:v}));
  const imgUp=(e)=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onloadend=()=>upd({...bk,image:r.result});r.readAsDataURL(f);};
  const addGrowth=()=>{if(!newGrowth.date)return;upd({...bk,growthRecords:[...gr,{...newGrowth,id:Date.now()}]});setNewGrowth({date:"",weight:"",bcs:0,price:"",notes:""});setModal("viewGrowth");};
  const addFeed=()=>{if(!newFeed.date||!newFeed.feedType)return;upd({...bk,feedRecords:[...fr,{...newFeed,id:Date.now()}]});setNewFeed({date:"",feedType:"",amount:"",minerals:"",notes:""});setModal("viewFeed");};
  const addMed=()=>{if(!newMed.date||!newMed.type)return;upd({...bk,medicalLog:[...ml,{...newMed,id:Date.now()}]});setNewMed({date:"",type:"",medicine:"",vetName:"",cost:"",notes:""});setModal("viewMed");};
  const addDrench=()=>{if(!newDrench.date||!newDrench.product)return;upd({...bk,drenchingRecords:[...dr,{...newDrench,id:Date.now()}]});setNewDrench({date:"",product:"",dose:"",famacha:0,weightAtDrenching:"",nextDate:"",notes:""});setModal("viewDrench");};
  const del=(f,rid)=>upd({...bk,[f]:(bk[f]||[]).filter((r)=>r.id!==rid)});
  const graduate=()=>{upd({...bk,type:"buck",graduated:true,age:bk.age?String((parseFloat(bk.age)/12).toFixed(1)):bk.age,breedingRecords:[]});setGradModal(false);navigate("/animals/goats/bucks");};

  const chartData=[...gr].sort((a,b)=>new Date(a.date)-new Date(b.date)).map((r)=>({date:r.date,weight:r.weight?parseFloat(r.weight):null}));
  const mColor=M_COLORS[bk.maturityStatus]||"bg-gray-400";
  const statusColor={Healthy:"bg-green-100 text-green-800",Sick:"bg-red-100 text-red-700","Under Treatment":"bg-amber-100 text-amber-700"}[bk.status]||"bg-gray-100 text-gray-600";

  return (
    <div className="bg-orange-50 flex flex-col">
      <div className="flex items-center gap-4 px-6 py-4 bg-white shadow-sm">
        <button onClick={()=>navigate(-1)} className="bg-white shadow w-9 h-9 rounded-full flex items-center justify-center hover:scale-110 transition"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4 text-orange-700"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5"/></svg></button>
        <h2 className="text-2xl font-bold text-orange-900">Buckling Profile</h2>
        <div className="ml-auto flex items-center gap-2">
          <span className={`text-xs font-semibold px-3 py-1 rounded-full ${statusColor}`}>{bk.status}</span>
          <span className={`text-xs font-semibold px-3 py-1 rounded-full text-white ${mColor}`}>{bk.maturityStatus||"Growing"}</span>
          {!bk.graduated&&parseFloat(bk.age)>=12&&<button onClick={()=>setGradModal(true)} className="text-xs font-semibold px-3 py-1 rounded-full bg-green-600 text-white hover:bg-green-700 transition">Graduate → Buck</button>}
        </div>
      </div>
      <div className="flex flex-1 gap-6 p-6 flex-wrap lg:flex-nowrap">
        <div className="flex flex-col gap-4 w-72 shrink-0">
          <div className="relative h-64 rounded-2xl overflow-hidden shadow-lg bg-gray-100 group">
            {bk.image?<img src={bk.image} alt={bk.name} className="w-full h-full object-cover"/>:<div className="flex items-center justify-center h-full text-gray-400 text-sm">No image</div>}
            <label className="absolute inset-0 flex items-end justify-center pb-4 bg-black/0 group-hover:bg-black/30 transition cursor-pointer"><span className="opacity-0 group-hover:opacity-100 bg-white/90 text-orange-800 text-xs font-semibold px-3 py-1 rounded-full transition">Change photo</span><input type="file" accept="image/*" className="hidden" onChange={imgUp}/></label>
          </div>
          <div className="bg-white rounded-2xl shadow p-4 text-sm space-y-2">
            {[{l:"Breed",f:"breed"},{l:"Color",f:"color"},{l:"Weight",f:"weight",s:" kg"},{l:"Age",f:"age",s:" mo"}].map(({l,f,s=""})=>(
              <div key={f} className="flex justify-between items-center border-b last:border-0 pb-1 last:pb-0">
                <span className="text-gray-500">{l}</span>
                {isEditing?<input value={edited[f]||""} onChange={(e)=>uf(f,e.target.value)} className="border rounded px-2 py-0.5 w-28 text-right text-sm"/>:<span className="text-orange-900 font-semibold">{bk[f]?`${bk[f]}${s}`:<span className="text-gray-300">—</span>}</span>}
              </div>
            ))}
          </div>
          <div className="bg-white rounded-2xl shadow p-4 text-sm">
            ...
            {["dam","sire"].map((field)=>(
              <div key={field} className="flex justify-between items-center border-b last:border-0 pb-1 last:pb-0">
                ...
                {isEditing?<select value={edited[field]||""} onChange={(e)=>uf(field,e.target.value)} className="border rounded px-2 py-0.5 w-36 text-sm"><option value="">Unknown</option>{(field==="dam"?does:bucks).map((a)=><option key={a.id} value={a.name}>{a.name}</option>)}</select>:<span className="text-orange-900 font-semibold">{bk[field]||<span className="text-gray-300">—</span>}</span>}
              </div>
            ))}
          </div>
          <div className="bg-white rounded-2xl shadow p-4 text-sm">
            <p className="font-semibold text-gray-500 text-xs uppercase mb-2">Maturity Progress</p>
            {isEditing?<div className="flex justify-between items-center"><span className="text-gray-500 text-xs">Target weight (kg)</span><input type="number" value={edited.targetWeight||""} onChange={(e)=>uf("targetWeight",e.target.value)} className="border rounded px-2 py-0.5 w-24 text-sm"/></div>:<MaturityBar current={bk.weight} target={bk.targetWeight}/>}
            {!isEditing&&!bk.targetWeight&&<p className="text-gray-300 text-xs">Set target weight in Edit Profile</p>}
          </div>
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div className="bg-white rounded-2xl shadow p-5">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1"><p className="text-xs font-semibold text-gray-400 uppercase mb-1">Name</p>{isEditing?<input value={edited.name||""} onChange={(e)=>uf("name",e.target.value)} className="text-2xl font-bold border-b border-orange-400 outline-none w-full"/>:<h3 className="text-2xl font-bold text-orange-900">{bk.name}</h3>}</div>
              {!isEditing?<button onClick={startEdit} className="bg-orange-600 text-white px-4 py-2 rounded-xl text-sm hover:bg-orange-700 transition">Edit Profile</button>:<div className="flex gap-2"><button onClick={saveEdit} className="bg-green-600 text-white px-4 py-2 rounded-xl text-sm">Save</button><button onClick={cancelEdit} className="bg-red-500 text-white px-4 py-2 rounded-xl text-sm">Cancel</button></div>}
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <ActionCard title="Growth" count={gr.length} accent="amber" latest={gr.length>0?`${gr.at(-1).weight||"—"} kg`:null} latestDate={gr.at(-1)?.date} onAdd={()=>setModal("growth")} onView={()=>setModal("viewGrowth")} iconPath="M2.25 18L9 11.25l4.306 4.307a11.95 11.95 0 015.814-5.519l2.74-1.22m0 0l-5.94-2.28m5.94 2.28l-2.28 5.941"/>
      ...
      {gradModal&&<div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"><div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6"><h3 className="text-lg font-bold text-green-900 mb-2">Graduate {bk.name} to Buck?</h3><p className="text-sm text-gray-600 mb-4">All records will carry across.</p><div className="flex gap-3"><button onClick={graduate} className="flex-1 bg-green-600 text-white py-2 rounded-xl hover:bg-green-700 font-semibold">Confirm</button><button onClick={()=>setGradModal(false)} className="flex-1 bg-gray-100 text-gray-700 py-2 rounded-xl">Cancel</button></div></div></div>}
    </div>
  );
}