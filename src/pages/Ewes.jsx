...
export default function Ewes() {
  ...
  const ewes = animals.filter(
    (a) => a.category === "sheep" && a.type?.toLowerCase() === "ewe"
  );
    ...
    <div className="bg-green-50 relative p-6">
      ...
      <div className="bg-white border rounded-2xl p-5 flex flex-wrap gap-3 shadow-sm mb-8">
        ...
        <button
          onClick={add}
          disabled={!name || !age}
          className="bg-rose-600 text-white px-5 py-3 rounded-xl hover:bg-rose-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition"
        >
          Add Ewe
        </button>
