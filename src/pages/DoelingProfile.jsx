...
const R_COLORS  = { Weaning:"bg-gray-400", Growing:"bg-amber-500", "Near Breeding Age":"bg-rose-400", "Ready to Breed":"bg-green-600" };

function ReadinessBar({current,target}) {
  if(!current||!target) return null;
  const pct=Math.min(100,Math.round((parseFloat(current)/parseFloat(target))*100));
  const col=pct<50?"bg-amber-400":pct<80?"bg-rose-400":"bg-green-500";
...
export default function DoelingProfile() {
  const {id}=useParams(); const navigate=useNavigate();
  const {animals=[],setAnimals}=useContext(FarmContext);
  const dl=animals.find((a)=>a.id?.toString()===id&&a.category==="goats"&&a.type?.toLowerCase()==="doeling");
  ...
  if(!dl) return <p className="p-6 text-red-600">Doeling not found.</p>;
  const gr=dl.growthRecords||[]; const fr=dl.feedRecords||[]; const ml=dl.medicalLog||[];
  const dr=dl.drenchingRecords||[]; const hr=dl.heatRecords||[];
  ...
  const startEdit=()=>{setIsEditing(true);setEdited({...dl});}; const cancelEdit=()=>{setIsEditing(false);setEdited(null);};
  const saveEdit=()=>{upd(edited);setIsEditing(false);setEdited(null);}; const uf=(f,v)=>setEdited((p)=>({...p,[f]:v}));
  const imgUp=(e)=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onloadend=()=>upd({...dl,image:r.result});r.readAsDataURL(f);};
  const addGrowth=()=>{if(!newGrowth.date)return;upd({...dl,growthRecords:[...gr,{...newGrowth,id:Date.now()}]});setNewGrowth({date:"",weight:"",bcs:0,price:"",notes:""});setModal("viewGrowth");};
  const addFeed=()=>{if(!newFeed.date||!newFeed.feedType)return;upd({...dl,feedRecords:[...fr,{...newFeed,id:Date.now()}]});setNewFeed({date:"",feedType:"",amount:"",minerals:"",notes:""});setModal("viewFeed");};
  const addMed=()=>{if(!newMed.date||!newMed.type)return;upd({...dl,medicalLog:[...ml,{...newMed,id:Date.now()}]});setNewMed({date:"",type:"",medicine:"",vetName:"",cost:"",notes:""});setModal("viewMed");};
  const addDrench=()=>{if(!newDrench.date||!newDrench.product)return;upd({...dl,drenchingRecords:[...dr,{...newDrench,id:Date.now()}]});setNewDrench({date:"",product:"",dose:"",famacha:0,weightAtDrenching:"",nextDate:"",notes:""});setModal("viewDrench");};
  const addHeat=()=>{if(!newHeat.date)return;upd({...dl,heatRecords:[...hr,{...newHeat,id:Date.now()}]});setNewHeat({date:"",duration:"",intensity:"",notes:""});setModal("viewHeat");};
  const del=(f,rid)=>upd({...dl,[f]:(dl[f]||[]).filter((r)=>r.id!==rid)});
  const graduate=()=>{upd({...dl,type:"doe",graduated:true,age:dl.age?String((parseFloat(dl.age)/12).toFixed(1)):dl.age,pregnancyStatus:"Open",milkRecords:[],kiddingRecords:[],matingRecords:[]});setGradModal(false);navigate("/animals/goats/does");};

  const avgCycle=hr.length>=2?(() => {
    const sorted=[...hr].sort((a,b)=>new Date(a.date)-new Date(b.date));
    const diffs=sorted.slice(1).map((r,i)=>Math.round((new Date(r.date)-new Date(sorted[i].date))/86400000));
  ...
  const rColor=R_COLORS[dl.readinessStatus]||"bg-gray-400";
  const statusColor={Healthy:"bg-green-100 text-green-800",Sick:"bg-red-100 text-red-700","Under Treatment":"bg-amber-100 text-amber-700"}[dl.status]||"bg-gray-100 text-gray-600";
    ...
    <div className="bg-rose-50 flex flex-col">
      <div className="flex items-center gap-4 px-6 py-4 bg-white shadow-sm">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {!dl.graduated&&parseFloat(dl.age)>=12&&<button onClick={()=>setGradModal(true)} className="text-xs font-semibold px-3 py-1 rounded-full bg-green-600 text-white hover:bg-green-700 transition">Graduate → Doe</button>}
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl shadow p-4 text-sm space-y-2">
            {[{l:"Breed",f:"breed"},{l:"Color",f:"color"},{l:"Weight",f:"weight",s:" kg"},{l:"Age",f:"age",s:" mo"}].map(({l,f,s=""})=>(
              <div key={f} className="flex justify-between items-center border-b last:border-0 pb-1 last:pb-0">
                ...
                {isEditing?<input value={edited[f]||""} onChange={(e)=>uf(f,e.target.value)} className="border rounded px-2 py-0.5 w-28 text-right text-sm"/>:<span className="text-rose-900 font-semibold">{dl[f]?`${dl[f]}${s}`:<span className="text-gray-300">—</span>}</span>}
              </div>
            ))}
          </div>
          <div className="bg-white rounded-2xl shadow p-4 text-sm">
            ...
            {["dam","sire"].map((field)=>(
              <div key={field} className="flex justify-between items-center border-b last:border-0 pb-1 last:pb-0">
                ...
                {isEditing?<select value={edited[field]||""} onChange={(e)=>uf(field,e.target.value)} className="border rounded px-2 py-0.5 w-36 text-sm"><option value="">Unknown</option>{(field==="dam"?does:bucks).map((a)=><option key={a.id} value={a.name}>{a.name}</option>)}</select>:<span className="text-rose-900 font-semibold">{dl[field]||<span className="text-gray-300">—</span>}</span>}
              </div>
            ))}
          </div>
          <div className="bg-white rounded-2xl shadow p-4 text-sm">
            <p className="font-semibold text-gray-500 text-xs uppercase mb-2">Breeding Readiness</p>
            {isEditing?<div className="flex justify-between items-center"><span className="text-gray-500 text-xs">Target weight (kg)</span><input type="number" value={edited.targetWeight||""} onChange={(e)=>uf("targetWeight",e.target.value)} className="border rounded px-2 py-0.5 w-24 text-sm"/></div>:<ReadinessBar current={dl.weight} target={dl.targetWeight}/>}
            {!isEditing&&!dl.targetWeight&&<p className="text-gray-300 text-xs">Set target weight in Edit Profile</p>}
          </div>
          <div className="bg-white rounded-2xl shadow p-4 text-sm">
            <p className="font-semibold text-gray-500 text-xs uppercase mb-2">Heat Cycle Summary</p>
            <div className="space-y-1">
              <div className="flex justify-between"><span className="text-gray-500">Heats observed</span><span className="font-bold text-rose-700">{hr.length}</span></div>
              {avgCycle&&<div className="flex justify-between"><span className="text-gray-500">Avg cycle</span><span className="font-bold text-rose-700">{avgCycle} days</span></div>}
              {hr.length>0&&<div className="flex justify-between"><span className="text-gray-500">Last observed</span><span className="font-bold text-rose-700">{hr.at(-1).date}</span></div>}
            </div>
          </div>
        </div>
