...
export default function EweLambs() {
  ...
  const eweLambs = animals.filter((a) => a.category === "sheep" && a.type?.toLowerCase() === "ewe-lamb");
  ...
  return (
    <div className="bg-green-50 relative p-6">
      ...
      <div className="bg-white border rounded-2xl p-5 flex flex-wrap gap-3 shadow-sm mb-8">
        ...
        <button onClick={add} className="bg-violet-600 text-white px-5 py-3 rounded-xl hover:bg-violet-700 transition">Add Ewe Lamb</button>
      </div>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {eweLambs.map((e) => (
          <div key={e.id} className="relative h-80 rounded-3xl overflow-hidden shadow-xl group">
            <img src={e.image} alt={e.name} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" onError={(ev)=>{ ev.target.src=`https://placehold.co/400x320/ede9fe/4c1d95?text=${e.name}`; }} />
