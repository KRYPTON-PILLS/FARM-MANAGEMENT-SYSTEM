...
export default function RamLambs() {
  ...
  const ramLambs = animals.filter((a) => a.category === "sheep" && a.type?.toLowerCase() === "ram-lamb");
  ...
  return (
    <div className="bg-green-50 relative p-6">
      ...
      <div className="bg-white border rounded-2xl p-5 flex flex-wrap gap-3 shadow-sm mb-8">
        ...
        <button onClick={add} className="bg-sky-600 text-white px-5 py-3 rounded-xl hover:bg-sky-700 transition">Add Ram Lamb</button>
      </div>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {ramLambs.map((r) => (
          <div key={r.id} className="relative h-80 rounded-3xl overflow-hidden shadow-xl group">
            <img src={r.image} alt={r.name} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" onError={(ev)=>{ ev.target.src=`https://placehold.co/400x320/e0f2fe/075985?text=${r.name}`; }} />
            <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent" />
            <div className="absolute top-3 left-3 bg-black/60 text-white px-3 py-1 text-xs rounded-full backdrop-blur-sm">{r.age} mo</div>
