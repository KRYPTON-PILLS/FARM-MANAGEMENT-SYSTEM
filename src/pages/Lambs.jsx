...
export default function Lambs() {
  ...
  const lambs = animals.filter((a) => a.category === "sheep" && a.type?.toLowerCase() === "lamb");
  ...
  const addLamb = () => {
    ...
    setAnimals((prev) => [...prev, {
      id: Date.now(), category: "sheep", type: "lamb",
  ...
  return (
    <div className="bg-green-50 relative p-6">
      ...
      <h2 className="text-3xl font-bold text-green-900 mb-6 ml-14">Lambs</h2>

      <div className="bg-white border rounded-2xl p-5 flex flex-wrap gap-3 shadow-sm mb-8">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Lamb Name" className="border p-3 rounded-lg flex-1 min-w-[180px]" />
        ...
        <button onClick={addLamb} className="bg-amber-500 text-white px-5 py-3 rounded-xl hover:bg-amber-600 transition">Add Lamb</button>
      </div>

      <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {lambs.map((l) => {
          const days = l.birthDate ? Math.floor((Date.now() - new Date(l.birthDate)) / 86400000) : null;
          const urgent = days !== null && days <= 3 && !l.colostrumRecord;
          return (
            <div key={l.id} className="relative h-80 rounded-3xl overflow-hidden shadow-xl group">
              <img src={l.image} alt={l.name} className="w-full h-full object-cover group-hover:scale-105 transition duration-500"
                onError={(e) => { e.target.src = `https://placehold.co/400x320/fef9c3/92400e?text=${l.name}`; }} />
              <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent" />
              <div className="absolute top-3 left-3 bg-black/60 text-white px-3 py-1 text-xs rounded-full backdrop-blur-sm">{ageLabel(l.birthDate)}</div>
              <div className={`absolute top-3 right-3 ${l.gender==="Male"?"bg-sky-500":"bg-rose-500"} text-white px-3 py-1 text-xs rounded-full`}>{l.gender}</div>
              ...
              <Link to={`/animals/sheep/lambs/${l.id}`}>
                <button className="absolute bottom-4 right-4 bg-amber-500 text-white px-5 py-2 rounded-xl hover:bg-amber-600 transition shadow-lg">{l.name}</button>
              </Link>
            </div>
          );
        })}
      </div>
      {lambs.length === 0 && <p className="text-gray-500 text-lg mt-4">No lambs recorded yet.</p>}
    </div>
  );
}