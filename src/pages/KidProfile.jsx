  ...
  return (
    <div className="bg-amber-50 flex flex-col">
      <div className="flex items-center gap-3 px-4 sm:px-6 py-3 sm:py-4 bg-white shadow-sm flex-wrap">
        ...
        <h2 className="text-xl sm:text-2xl font-bold text-amber-900">Kid Profile</h2>
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div className="bg-white rounded-2xl shadow p-5">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1"><p className="text-xs font-semibold text-gray-400 uppercase mb-1">Name</p>{isEditing?<input value={edited.name||""} onChange={(e)=>uf("name",e.target.value)} className="text-xl sm:text-2xl font-bold border-b border-amber-400 outline-none w-full"/>:<h3 className="text-xl sm:text-2xl font-bold text-amber-900">{kid.name}</h3>}</div>
      ...
      {gradModal&&<div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center"><div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6"><h3 className="text-lg font-bold text-green-900 mb-2">Graduate {kid.name}?</h3><p className="text-sm text-gray-600 mb-3">Move to <strong>{kid.gender==="Male"?"Bucklings":"Doelings"}</strong>. All records kept.</p><div className="bg-green-50 border border-green-200 rounded-xl p-3 text-xs text-green-800 mb-4">{kid.gender==="Male"?"♂ Male → Bucklings (3–12 months)":"♀ Female → Doelings (3–12 months)"}</div><div className="flex gap-3"><button onClick={graduate} className="flex-1 bg-green-600 text-white py-2 rounded-xl hover:bg-green-700 font-semibold">Confirm</button><button onClick={()=>setGradModal(false)} className="flex-1 bg-gray-100 text-gray-700 py-2 rounded-xl">Cancel</button></div></div></div>}
    </div>
  );
}