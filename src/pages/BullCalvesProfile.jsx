...
const METRICS = [
  { key: "weight", label: "Weight (kg)", color: "#2563eb" },
  { key: "price",  label: "Est. Price",  color: "#d97706" },
  { key: "health", label: "Health",      color: "#16a34a" },
];

const MATURITY_STATUSES = ["Weaning", "Growing", "Near Maturity", "Ready"];
...
function MaturityBar({ current, target }) {
  ...
  const color = pct < 50 ? "bg-amber-400" : pct < 80 ? "bg-blue-500" : "bg-green-500";
  return (
    <div>
      <div className="flex justify-between text-xs text-gray-500 mb-1">
        <span>Weight progress to maturity</span>
...
export default function BullCalvesProfile() {
  const { id } = useParams();
  ...
  const [newCastration, setNewCastration] = useState({ date:"", method:"", vetName:"", cost:"", notes:"" });
    ...
    <div className="bg-green-50 flex flex-col">
      ...
      <div className="flex flex-1 gap-6 p-4 sm:p-6 flex-col lg:flex-row">
        ...
        <div className="flex flex-col gap-4 w-full lg:w-72 lg:shrink-0">
          ...
          {/* MATURITY PROGRESS */}
          <div className="bg-white rounded-2xl shadow p-4 text-sm">
            <p className="font-semibold text-gray-500 text-xs uppercase mb-3">Maturity Progress</p>
            {isEditing
              ? (
          ...
          <div className="bg-white rounded-2xl shadow p-4 text-sm">
            ...
            {calf.castrated && calf.castrationRecord ? (
              ...
              <button onClick={() => setModal("castration")}
                className="w-full bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs py-2 rounded-lg transition">
                Record castration
              </button>
            )}
          </div>
        ...
        <div className="flex-1 flex flex-col gap-4">
            ...
            {isEditing && (
              <div className="mt-4 flex gap-4 flex-wrap">
                ...
                <div>
                  ...
                  <select value={edited.maturityStatus || ""} onChange={(e) => updateField("maturityStatus", e.target.value)} className="border rounded-lg px-3 py-2 text-sm">
                    {MATURITY_STATUSES.map((s) => <option key={s}>{s}</option>)}
                  </select>
                </div>
              </div>
            )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
            ...
            {[
              ...
              const ACCENT = {
                blue:  { bg:"bg-blue-50",  iconColor:"#2563eb", count:"text-blue-800",  add:"bg-blue-600 hover:bg-blue-700",   view:"bg-blue-50 text-blue-700 hover:bg-blue-100" },
            ...
            <div className="bg-white rounded-2xl shadow p-5">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={chartData}>
                  <CartesianGrid strokeDasharray="4 4" stroke="#eff6ff" />
                    ...
                    <table className="w-full text-sm min-w-[560px]">
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {[...growthRecords].reverse().map((r, i) => (
                          <tr key={r.id} className={`hover:bg-blue-50/40 transition ${i % 2 === 0 ? "bg-white" : "bg-gray-50/40"}`}>
                            <td className="px-4 py-2.5 text-xs text-gray-700 font-medium whitespace-nowrap">{r.date}</td>
                            <td className="px-4 py-2.5 whitespace-nowrap">{r.weight ? <span className="font-bold text-blue-700">{parseFloat(r.weight).toLocaleString()} kg</span> : <span className="text-gray-300">—</span>}</td>
      ...
      {modal === "growth" && (
        <Modal title="Add Growth Record" onClose={() => setModal(null)}>
          ...
          <Field label="Weight (kg)"><input type="number" placeholder="e.g. 180" value={newGrowth.weight} onChange={(e) => setNewGrowth({...newGrowth, weight:e.target.value})} className="border rounded-lg p-2 w-full" /></Field>
          <Field label="Estimated Price (KES)"><input type="number" placeholder="e.g. 45000" value={newGrowth.price} onChange={(e) => setNewGrowth({...newGrowth, price:e.target.value})} className="border rounded-lg p-2 w-full" /></Field>
      ...
      {modal === "feed" && (
        <Modal title="Add Feed Record" onClose={() => setModal(null)}>
          ...
          <Field label="Feed Type"><input placeholder="e.g. Hay, Starter meal, Silage" value={newFeed.feedType} onChange={(e) => setNewFeed({...newFeed, feedType:e.target.value})} className="border rounded-lg p-2 w-full" /></Field>
          ...
          <Field label="Minerals / Supplements"><input placeholder="e.g. Calcium, Zinc" value={newFeed.minerals} onChange={(e) => setNewFeed({...newFeed, minerals:e.target.value})} className="border rounded-lg p-2 w-full" /></Field>
          <Field label="Weaning / Transition Diet Notes"><textarea placeholder="Notes on transition from milk to solid feed..." value={newFeed.transitionDiet} onChange={(e) => setNewFeed({...newFeed, transitionDiet:e.target.value})} className="border rounded-lg p-2 w-full h-16 resize-none" /></Field>
          <div className="flex gap-3 pt-2">
              ...
              {[...feedRecords].reverse().map((r) => (
                <div key={r.id} className="bg-amber-50 rounded-xl p-3 flex justify-between items-start">
                  <div className="space-y-0.5">
                    ...
                    {r.transitionDiet && <p className="text-xs text-blue-600 italic">Transition: {r.transitionDiet}</p>}
                  </div>
        ...
        <Modal title="Add Medical Record" onClose={() => setModal(null)}>
          ...
          <Field label="Type">
            <select value={newMedical.type} onChange={(e) => setNewMedical({...newMedical, type:e.target.value})} className="border rounded-lg p-2 w-full">
              ...
              {["Vaccination","Deworming","Treatment","Checkup","Navel Dressing","Vitamin Injection","Other"].map((t) => <option key={t}>{t}</option>)}
            </select>
          </Field>
          <Field label="Medicine / Vaccine"><input placeholder="e.g. Ivermectin, Blackleg vaccine" value={newMedical.medicine} onChange={(e) => setNewMedical({...newMedical, medicine:e.target.value})} className="border rounded-lg p-2 w-full" /></Field>
      ...
      {modal === "castration" && (
        <Modal title="Record Castration" onClose={() => setModal(null)}>
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4 text-xs text-amber-800">
            Once saved this is permanent and will mark this bull calf as castrated.
          </div>
          <Field label="Date of Castration"><input type="date" value={newCastration.date} onChange={(e) => setNewCastration({...newCastration, date:e.target.value})} className="border rounded-lg p-2 w-full" /></Field>
          <Field label="Method">
            <select value={newCastration.method} onChange={(e) => setNewCastration({...newCastration, method:e.target.value})} className="border rounded-lg p-2 w-full">
              <option value="">Select method...</option>
              {["Surgical","Burdizzo (Bloodless)","Rubber Ring","Other"].map((m) => <option key={m}>{m}</option>)}
            </select>
          </Field>
          <Field label="Vet Name"><input value={newCastration.vetName} onChange={(e) => setNewCastration({...newCastration, vetName:e.target.value})} className="border rounded-lg p-2 w-full" /></Field>
          <Field label="Cost (KES)"><input type="number" value={newCastration.cost} onChange={(e) => setNewCastration({...newCastration, cost:e.target.value})} className="border rounded-lg p-2 w-full" /></Field>
          <Field label="Notes"><textarea value={newCastration.notes} onChange={(e) => setNewCastration({...newCastration, notes:e.target.value})} className="border rounded-lg p-2 w-full h-16 resize-none" /></Field>
          <div className="flex gap-3 pt-2">
            <button onClick={saveCastration} className="flex-1 bg-gray-700 text-white py-2 rounded-xl hover:bg-gray-800 font-semibold">Confirm &amp; Save</button>
            <button onClick={() => setModal(null)} className="flex-1 bg-gray-100 text-gray-700 py-2 rounded-xl">Cancel</button>
          </div>
        </Modal>
      )}
