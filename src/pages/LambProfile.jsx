...
function ageStr(birthDate) {
  if (!birthDate) return "—";
  const d = Math.floor((Date.now()-new Date(birthDate))/86400000);
  if (d < 0) return "Not yet born";
...
function WeaningBar({ birthDate, targetDate }) {
  ...
  return (
    <div>
      <div className="flex justify-between text-xs text-gray-500 mb-1"><span>Progress to weaning</span><span className="font-bold">{pct}%</span></div>
      <div className="h-3 bg-gray-100 rounded-full overflow-hidden"><div className={`h-full ${col} rounded-full transition-all`} style={{width:`${pct}%`}} /></div>
      <p className="text-xs text-gray-400 mt-1">{rem>0?`${rem} days left`:rem===0?"Weaning day!":`${Math.abs(rem)} days overdue`}</p>
    </div>
  );
}

export default function LambProfile() {
  const { id } = useParams(); const navigate = useNavigate();
  const { animals=[], setAnimals } = useContext(FarmContext);
  const lamb = animals.find((a)=>a.id?.toString()===id&&a.category==="sheep"&&a.type?.toLowerCase()==="lamb");
  ...
  const [newMilk,setNewMilk]=useState({date:"",time:"",amount:"",feedType:"Ewe Milk",notes:""});
  const [newMed,setNewMed]=useState({date:"",type:"",medicine:"",vetName:"",cost:"",notes:""});

  if (!lamb) return <p className="p-6 text-red-600">Lamb not found.</p>;
  const growthRecords=lamb.growthRecords||[]; const milkFeedRecords=lamb.milkFeedRecords||[]; const medicalLog=lamb.medicalLog||[];
  const dv=daysOld(lamb.birthDate);
  ...
  const delGrowth=(rid)=>update({...lamb,growthRecords:growthRecords.filter((r)=>r.id!==rid)});
  const addMilk=()=>{ if(!newMilk.date) return; const u=[...milkFeedRecords,{...newMilk,id:Date.now()}]; update({...lamb,milkFeedRecords:u}); setNewMilk({date:"",time:"",amount:"",feedType:"Ewe Milk",notes:""}); setModal("viewMilk"); };
  const delMilk=(rid)=>update({...lamb,milkFeedRecords:milkFeedRecords.filter((r)=>r.id!==rid)});
  const addMed=()=>{ if(!newMed.date||!newMed.type) return; const u=[...medicalLog,{...newMed,id:Date.now()}]; update({...lamb,medicalLog:u}); setNewMed({date:"",type:"",medicine:"",vetName:"",cost:"",notes:""}); setModal("viewMed"); };
  const delMed=(rid)=>update({...lamb,medicalLog:medicalLog.filter((r)=>r.id!==rid)});

  const graduate=()=>{ const to=lamb.gender==="Male"?"ram-lamb":"ewe-lamb"; update({...lamb,type:to,graduated:true}); setGradModal(false); navigate(`/animals/sheep/${to==="ram-lamb"?"ram-lambs":"ewe-lambs"}`); };

  const chartData=[...growthRecords].sort((a,b)=>new Date(a.date)-new Date(b.date)).map((r)=>({date:r.date,weight:r.weight?parseFloat(r.weight):null}));
  const today=new Date().toISOString().split("T")[0];
  const todayMilk=milkFeedRecords.filter((r)=>r.date===today).reduce((s,r)=>s+(parseFloat(r.amount)||0),0);
  const statusColor={Healthy:"bg-green-100 text-green-800",Sick:"bg-red-100 text-red-700","Under Treatment":"bg-amber-100 text-amber-700"}[lamb.status]||"bg-gray-100 text-gray-600";
    ...
    <div className="bg-amber-50 flex flex-col">
      ...
      <div className="flex items-center gap-3 px-4 sm:px-6 py-3 sm:py-4 bg-white shadow-sm flex-wrap">
        <button onClick={()=>navigate(-1)} className="bg-white shadow w-9 h-9 rounded-full flex items-center justify-center hover:scale-110 transition">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4 text-amber-700"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" /></svg>
        </button>
        <h2 className="text-xl sm:text-2xl font-bold text-amber-900">Lamb Profile</h2>
        <span className="text-sm text-gray-500 font-medium">{ageStr(lamb.birthDate)}</span>
        <div className="ml-auto flex items-center gap-2">
          <span className={`text-xs font-semibold px-3 py-1 rounded-full ${statusColor}`}>{lamb.status}</span>
          <span className={`text-xs font-semibold px-3 py-1 rounded-full text-white ${lamb.gender==="Male"?"bg-sky-500":"bg-rose-500"}`}>{lamb.gender||"Unknown"}</span>
          {!lamb.graduated && dv>=180 && <button onClick={()=>setGradModal(true)} className="text-xs font-semibold px-3 py-1 rounded-full bg-green-600 text-white hover:bg-green-700 transition">Graduate →</button>}
        </div>
      </div>
      {dv!==null&&dv<=2&&!lamb.colostrumRecord&&<div className="bg-red-600 text-white text-center text-sm font-semibold py-2">🚨 Colostrum must be given within 24 hours of birth!</div>}
      ...
      <div className="flex flex-1 gap-6 p-4 sm:p-6 flex-col lg:flex-row">
          ...
          <div className="bg-white rounded-2xl shadow p-4 text-sm space-y-2">
            ...
            {[{l:"Birth date",f:"birthDate",t:"date"},{l:"Birth weight",f:"birthWeight",s:" kg"},{l:"Breed",f:"breed"},{l:"Color",f:"color"}].map(({l,f,s="",t="text"})=>(
              <div key={f} className="flex justify-between items-center border-b last:border-0 pb-1 last:pb-0">
                ...
                {isEditing?<input type={t} value={edited[f]||""} onChange={(e)=>uf(f,e.target.value)} className="border rounded px-2 py-0.5 w-32 text-right text-sm"/>:<span className="text-amber-900 font-semibold">{lamb[f]?`${lamb[f]}${s}`:<span className="text-gray-300">—</span>}</span>}
              </div>
            ))}
          </div>
          <div className="bg-white rounded-2xl shadow p-4 text-sm">
            ...
            {["dam","sire"].map((field)=>(
              <div key={field} className="flex justify-between items-center border-b last:border-0 pb-1 last:pb-0">
                ...
                {isEditing?<select value={edited[field]||""} onChange={(e)=>uf(field,e.target.value)} className="border rounded px-2 py-0.5 w-36 text-sm"><option value="">Unknown</option>{(field==="dam"?ewes:rams).map((a)=><option key={a.id} value={a.name}>{a.name}</option>)}</select>:<span className="text-amber-900 font-semibold">{lamb[field]||<span className="text-gray-300">—</span>}</span>}
              </div>
            ))}
          </div>
          <div className="bg-white rounded-2xl shadow p-4 text-sm">
            <p className="font-semibold text-gray-500 text-xs uppercase mb-2">Colostrum</p>
            {lamb.colostrumRecord?(
              <div className="bg-green-50 border border-green-200 rounded-xl p-3 text-xs">
                <p className="font-bold text-green-800 mb-1">✅ Recorded</p>
                <p className="text-gray-600">Time: {lamb.colostrumRecord.timeAfterBirth} hrs after birth</p>
                <p className="text-gray-600">Amount: {lamb.colostrumRecord.amount} mL · Source: {lamb.colostrumRecord.source}</p>
              </div>
            ):(
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div className="bg-white rounded-2xl shadow p-5">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                ...
                {isEditing?<input value={edited.name||""} onChange={(e)=>uf("name",e.target.value)} className="text-xl sm:text-2xl font-bold border-b border-amber-400 outline-none w-full"/>:<h3 className="text-xl sm:text-2xl font-bold text-amber-900">{lamb.name}</h3>}
              </div>
          ...
          {milkFeedRecords.length>0&&<div className="bg-amber-100 rounded-2xl p-4 flex items-center justify-between"><div><p className="text-xs font-semibold text-amber-700 uppercase">Today's milk intake</p><p className="text-xl sm:text-2xl font-bold text-amber-900">{todayMilk>0?`${todayMilk} mL`:"Not fed yet today"}</p></div><div className="text-4xl">🍼</div></div>}

          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
            <ActionCard title="Growth Records" count={growthRecords.length} accent="amber" latest={growthRecords.length>0?`${growthRecords.at(-1).weight||"—"} kg`:null} latestDate={growthRecords.at(-1)?.date} onAdd={()=>setModal("growth")} onView={()=>setModal("viewGrowth")} iconPath="M2.25 18L9 11.25l4.306 4.307a11.95 11.95 0 015.814-5.519l2.74-1.22m0 0l-5.94-2.28m5.94 2.28l-2.28 5.941"/>
            <ActionCard title="Milk Feedings" count={milkFeedRecords.length} accent="green" latest={milkFeedRecords.length>0?`${milkFeedRecords.at(-1).amount||"—"} mL`:null} latestDate={milkFeedRecords.at(-1)?.date} onAdd={()=>setModal("milk")} onView={()=>setModal("viewMilk")} iconPath="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 014.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15M14.25 3.104c.251.023.501.05.75.082M19.8 15l-1.575 1.399A1.5 1.5 0 0117 17.58v.82A2.25 2.25 0 0114.75 20.625H9.25A2.25 2.25 0 017 18.4v-.82a1.5 1.5 0 01-.225-1.181L5 14.5m14.8.5l-5.8-4.5M5 14.5l5.8-4.5"/>
            <ActionCard title="Medical Log" count={medicalLog.length} accent="red" latest={medicalLog.length>0?medicalLog.at(-1).type:null} latestDate={medicalLog.at(-1)?.date} onAdd={()=>setModal("med")} onView={()=>setModal("viewMed")} iconPath="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25z"/>
          </div>
      ...
      {modal==="growth"&&<Modal title="Add Growth Record" onClose={()=>setModal(null)}><Field label="Date"><input type="date" value={newGrowth.date} onChange={(e)=>setNewGrowth({...newGrowth,date:e.target.value})} className="border rounded-lg p-2 w-full"/></Field><Field label="Weight (kg)"><input type="number" step="0.1" placeholder="e.g. 8.5" value={newGrowth.weight} onChange={(e)=>setNewGrowth({...newGrowth,weight:e.target.value})} className="border rounded-lg p-2 w-full"/></Field><Field label="Body Condition Score (BCS)"><BCSPicker value={newGrowth.bcs} onChange={(v)=>setNewGrowth({...newGrowth,bcs:v})}/></Field><Field label="Notes"><textarea value={newGrowth.notes} onChange={(e)=>setNewGrowth({...newGrowth,notes:e.target.value})} className="border rounded-lg p-2 w-full h-16 resize-none"/></Field><div className="flex gap-3 pt-2"><button onClick={addGrowth} className="flex-1 bg-amber-500 text-white py-2 rounded-xl hover:bg-amber-600 font-semibold">Save</button><button onClick={()=>setModal(null)} className="flex-1 bg-gray-100 text-gray-700 py-2 rounded-xl">Cancel</button></div></Modal>}
      {modal==="viewGrowth"&&<Modal title={`Growth Records — ${growthRecords.length}`} onClose={()=>setModal(null)}><button onClick={()=>setModal("growth")} className="mb-4 bg-amber-500 text-white px-4 py-2 rounded-xl text-sm hover:bg-amber-600">+ Add</button>{growthRecords.length===0?<p className="text-gray-400 text-center py-8">No records yet.</p>:<div className="space-y-3">{[...growthRecords].reverse().map((r)=><div key={r.id} className="bg-amber-50 rounded-xl p-3 flex justify-between"><div><p className="font-semibold text-amber-900 text-sm">{r.date}</p>{r.weight&&<p className="text-xs text-gray-600">⚖️ {r.weight} kg</p>}{r.bcs>0&&<BCSBadge score={r.bcs}/>}{r.notes&&<p className="text-xs text-gray-500 italic">{r.notes}</p>}</div><button onClick={()=>delGrowth(r.id)} className="text-red-400 hover:text-red-600 text-lg ml-3">&times;</button></div>)}</div>}</Modal>}

      {modal==="milk"&&<Modal title="Record Milk Feeding" onClose={()=>setModal(null)}><Field label="Date"><input type="date" value={newMilk.date} onChange={(e)=>setNewMilk({...newMilk,date:e.target.value})} className="border rounded-lg p-2 w-full"/></Field><Field label="Time"><input type="time" value={newMilk.time} onChange={(e)=>setNewMilk({...newMilk,time:e.target.value})} className="border rounded-lg p-2 w-full"/></Field><Field label="Amount (mL)"><input type="number" placeholder="e.g. 200" value={newMilk.amount} onChange={(e)=>setNewMilk({...newMilk,amount:e.target.value})} className="border rounded-lg p-2 w-full"/></Field><Field label="Feed Type"><div className="flex flex-wrap gap-2">{["Ewe Milk","Milk Replacer","Starter Pellets","Mixed"].map((t)=><button key={t} type="button" onClick={()=>setNewMilk({...newMilk,feedType:t})} className={`px-3 py-2 rounded-xl border text-xs font-semibold transition ${newMilk.feedType===t?"bg-green-600 text-white border-transparent":"bg-gray-50 border-gray-200 hover:border-gray-400"}`}>{t}</button>)}</div></Field><Field label="Notes"><textarea value={newMilk.notes} onChange={(e)=>setNewMilk({...newMilk,notes:e.target.value})} className="border rounded-lg p-2 w-full h-14 resize-none"/></Field><div className="flex gap-3 pt-2"><button onClick={addMilk} className="flex-1 bg-green-600 text-white py-2 rounded-xl hover:bg-green-700 font-semibold">Save</button><button onClick={()=>setModal(null)} className="flex-1 bg-gray-100 text-gray-700 py-2 rounded-xl">Cancel</button></div></Modal>}
      {modal==="viewMilk"&&<Modal title={`Milk Feedings — ${milkFeedRecords.length}`} onClose={()=>setModal(null)}><button onClick={()=>setModal("milk")} className="mb-4 bg-green-600 text-white px-4 py-2 rounded-xl text-sm hover:bg-green-700">+ Record</button>{milkFeedRecords.length===0?<p className="text-gray-400 text-center py-8">No feedings yet.</p>:<div className="space-y-3">{[...milkFeedRecords].reverse().map((r)=><div key={r.id} className="bg-green-50 rounded-xl p-3 flex justify-between"><div><p className="font-semibold text-green-900">{r.date}{r.time?` at ${r.time}`:""}</p><p className="text-xs text-gray-600">{r.feedType}{r.amount?` · ${r.amount} mL`:""}</p>{r.notes&&<p className="text-xs text-gray-500 italic">{r.notes}</p>}</div><button onClick={()=>delMilk(r.id)} className="text-red-400 hover:text-red-600 text-lg ml-3">&times;</button></div>)}</div>}</Modal>}
      ...
      {modal==="viewMed"&&<Modal title={`Medical Log — ${medicalLog.length}`} onClose={()=>setModal(null)}><button onClick={()=>setModal("med")} className="mb-4 bg-red-600 text-white px-4 py-2 rounded-xl text-sm hover:bg-red-700">+ Add</button>{medicalLog.length===0?<p className="text-gray-400 text-center py-8">No records yet.</p>:<div className="space-y-3">{[...medicalLog].reverse().map((r)=><div key={r.id} className="bg-red-50 rounded-xl p-3 flex justify-between"><div><p className="font-semibold text-red-900">{r.type}</p><p className="text-xs text-gray-500">{r.date}{r.vetName?` · ${r.vetName}`:""}</p>{r.medicine&&<p className="text-xs text-gray-600">Product: {r.medicine}</p>}{r.cost&&<p className="text-xs text-gray-600">Cost: KES {r.cost}</p>}{r.notes&&<p className="text-xs text-gray-500 italic">{r.notes}</p>}</div><button onClick={()=>delMed(r.id)} className="text-red-400 hover:text-red-600 text-lg ml-3">&times;</button></div>)}</div>}</Modal>}

      {gradModal&&<div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center"><div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6"><h3 className="text-lg font-bold text-green-900 mb-2">Graduate {lamb.name}?</h3><p className="text-sm text-gray-600 mb-4">Move to <strong>{lamb.gender==="Male"?"Ram Lambs":"Ewe Lambs"}</strong>. All records kept.</p><div className="bg-green-50 border border-green-200 rounded-xl p-3 text-xs text-green-800 mb-4">{lamb.gender==="Male"?"♂ Male → Ram Lambs (6–12 months)":"♀ Female → Ewe Lambs (6–12 months)"}</div><div className="flex gap-3"><button onClick={graduate} className="flex-1 bg-green-600 text-white py-2 rounded-xl hover:bg-green-700 font-semibold">Confirm</button><button onClick={()=>setGradModal(false)} className="flex-1 bg-gray-100 text-gray-700 py-2 rounded-xl">Cancel</button></div></div></div>}
    </div>
  );
}